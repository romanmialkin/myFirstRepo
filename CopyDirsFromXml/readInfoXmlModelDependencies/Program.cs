...
namespace readInfoXmlModelDependencies
{
    class Program
    {
        ...
        static void ReadInfoXml()
        {
            var xml = XDocument.Load(@"f:\git\NemoDory\Dory\User\Object\BARGE404\BARGE404.info.xml");
        ...
        {
            var proc = Process.GetProcesses(".").OrderBy(c => c.Id).Select(c => c);

            foreach (var rp in proc)
            {
                Console.WriteLine($"->PID: {rp.Id}\t Name: {rp.ProcessName}");
            }
        }
    }
}
